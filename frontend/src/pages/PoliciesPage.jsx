  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between border-b border-[#1E293B] pb-4">
        <div>
          <h2 className="text-2xl font-bold font-['Outfit'] text-[#F8FAFC]">Merchant Recovery Policies</h2>
      ...
      {saveSuccess && (
        <div className="p-4 rounded-xl bg-[#10B981]/12 border border-[#10B981]/25 text-[#10B981] text-xs font-semibold flex items-center space-x-2">
      ...
      {/* Policy Form */}
      <form onSubmit={handleSubmit} className="rounded-xl bg-[#171E2E] border border-[#1E293B] p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Max Retries */}
          <div className="space-y-2">
            <label className="text-xs font-semibold text-[#F8FAFC]">Max Automatic Payment Retries</label>
            <input
              type="number"
              min="0"
              max="10"
              ...
              className="w-full bg-[#0F172A] border border-[#1E293B] rounded-lg px-3 py-2 text-xs text-[#F8FAFC] focus:border-[#3B82F6] focus:outline-none"
            />
            <p className="text-[11px] text-[#64748B]">Maximum retry attempts permitted per failed transaction.</p>
          </div>

          {/* Max Contacts */}
          <div className="space-y-2">
            <label className="text-xs font-semibold text-[#F8FAFC]">Max Customer Contact Messages</label>
            <input
              type="number"
              min="0"
              max="10"
              ...
              className="w-full bg-[#0F172A] border border-[#1E293B] rounded-lg px-3 py-2 text-xs text-[#F8FAFC] focus:border-[#3B82F6] focus:outline-none"
            />
            <p className="text-[11px] text-[#64748B]">Maximum customer communications allowed per recovery case.</p>
          </div>

          {/* Recovery Window Days */}
          <div className="space-y-2">
            <label className="text-xs font-semibold text-[#F8FAFC]">Recovery Window (Days)</label>
            <input
              type="number"
              min="1"
              max="30"
              ...
              className="w-full bg-[#0F172A] border border-[#1E293B] rounded-lg px-3 py-2 text-xs text-[#F8FAFC] focus:border-[#3B82F6] focus:outline-none"
            />
            <p className="text-[11px] text-[#64748B]">Maximum duration in days before closing an unrecovered case.</p>
          </div>

          {/* Minimum EV Threshold */}
          <div className="space-y-2">
            <label className="text-xs font-semibold text-[#F8FAFC]">Minimum EV Threshold (₹)</label>
            <input
              type="number"
              min="0"
              ...
              className="w-full bg-[#0F172A] border border-[#1E293B] rounded-lg px-3 py-2 text-xs text-[#F8FAFC] focus:border-[#3B82F6] focus:outline-none"
            />
            <p className="text-[11px] text-[#64748B]">Cases below this expected recovery value will be stopped immediately.</p>
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-[#1E293B] flex items-center justify-between">
          <div>
            <h4 className="text-xs font-semibold text-[#F8FAFC]">Human Escalation Queue</h4>
            <p className="text-[11px] text-[#94A3B8]">Escalate cases to finance/support team when AI confidence is low.</p>
          </div>
          <button
            type="button"
            onClick={() => setFormData({ ...formData, humanEscalationEnabled: !formData.humanEscalationEnabled })}
            className={`w-12 h-6 flex items-center rounded-full p-1 transition-colors ${
              formData.humanEscalationEnabled ? 'bg-[#2563EB] justify-end' : 'bg-[#1E293B] border border-[#334155] justify-start'
            }`}
          >
        ...
        <div className="pt-4 border-t border-[#1E293B] space-y-3">
          <h4 className="text-xs font-semibold text-[#F8FAFC]">Allowed Intervention Channels</h4>
            ...
            {['retry', 'payment_link', 'email'].map((ch) => (
              <label key={ch} className="inline-flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  className="rounded bg-[#0F172A] border-[#1E293B] text-[#2563EB] focus:ring-0"
                />
                <span className="capitalize text-[#94A3B8] font-mono">{ch.replace('_', ' ')}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-[#1E293B] flex justify-end">
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-lg bg-[#2563EB] hover:bg-[#3B82F6] text-[#F8FAFC] text-xs font-semibold shadow-lg shadow-[#2563EB]/20 transition-colors disabled:opacity-50"
          >
